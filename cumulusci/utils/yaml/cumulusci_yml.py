...
class ReleaseBranchFormat(CCIDictModel):
    ...
    max_sprints_per_quarter: Optional[int] = 6  # For FYyyQqSn only, defaults to 6
    ...
    def validate_max_sprints(cls, v, values):
        if values.get("type") == "date":
            ...
            if "q" in values.get("pattern"):
                if v is None:
                    v = 6
                elif not (1 <= v <= 10):
                    raise ValueError(
                        f"max_sprints_per_quarter must be 1-10 when pattern contains 'q', got {v}"
                    )
        return v
