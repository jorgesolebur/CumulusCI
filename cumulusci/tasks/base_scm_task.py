        ...
        for service_type in services:
            for service in services.get(service_type, []):
                ...
                if service_type == "azure_devops":
                    ...
                    server_domain = server_url.split("/")[
                        0
                    ]  # TODO: Implement proper mechanism, url format dev.azure.com/{ORGANIZATION}.
                else:
                ...
                if server_domain and server_domain in runtime.project_config.repo_url:
                    if service_type == "azure_devops":
                        module = importlib.import_module(
                            "cumulusci.tasks.azure_devops.base", "BaseAzureTask"
                        )
                        self.scmKlass = module.BaseAzureTask
                    else:
                        # For Github Enterprise
                        module = importlib.import_module(
                            "cumulusci.tasks.github.base", "BaseGithubTask"
                        )
